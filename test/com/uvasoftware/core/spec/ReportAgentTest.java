package com.uvasoftware.core.spec;
/*
...
public class ReportAgentTest {
	...
	private static class Oracle implements IEventHandler {
		...
		public Oracle() {
			...
			p.setProperty(ConfigConstants.AGENT_ZONE_URL, Config.getInstance().get(ConfigConstants.TEST_ZIS));
	...
	@Test
	public void testAReportingAgent() throws Exception {
		...
		if (ack.getStatusCode() != Code.Status.SUCCESS) {
			...
			try {
				...
				assertEquals("John",sp.getName().getFirstName());
				assertEquals("Smith",sp.getName().getLastName());
